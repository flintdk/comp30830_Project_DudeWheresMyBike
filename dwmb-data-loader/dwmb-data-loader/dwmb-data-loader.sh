#!/bin/bash
# DWMB-Data-Loader.sh; Wrapper script to start, stop and display the DWMB-Data-Loader job
#
...
cron_dir="/etc/cron.d"
#cron_dir="/etc/cron.d"
module_to_schedule="${home_dir}/github/comp30830_project_2022/dwmb-data-loader/data_loader.py"
...
function echoClientCommandDocs() {
    #cmdMsg="DWMB-Data-Loader.sh Commands Processed:\n"
    cmdMsg="  \e[3mhelp\e[0m - See this help text\n"
    cmdMsg+="  \e[3mshow\e[0m - Show the current state of the Cron table\n"
    cmdMsg+="  \e[3mschedule\e[0m - Schedule the DWMB-Data-Loader to run, using default timings\n"
    cmdMsg+="  \e[3mstop\e[0m - Remove the DWMB-Data-Loader from the Cron scheduler\n"
    echo -e "$cmdMsg"
}
...
if [ -z "$1" ]; then
    usage 1 "DWMB-Data-Loader.sh: ERROR You must supply an instruction.";
fi
...
help)
    ...
    echo -e "DWMB-Data-Loader.sh: The DWMB-Data-Loader supports the following Commands:"
    echoClientCommandDocs
    ;;
show)
    # show: print out the current crontab table
    #crontab -l
    if [ -e "${cron_dir}/dwmb-data-loader" ]; then
        echo "DWMB-Data-Loader.sh: The current state of the Crontab is as follows:"
        cat "${cron_dir}/dwmb-data-loader"
    else
        echo "DWMB-Data-Loader.sh: ERROR schedule not found! Aborting..."
        exit 1
    fi
    ;;
schedule)
    ...
    echo "DWMB-Data-Loader.sh: Creating cron file..."
    touch "${cron_dir}/dwmb-data-loader"
    #
    ...
    # echo SHELL=/bin/sh >> "${cron_dir}/dwmb-data-loader"
    # PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin >> "${cron_dir}/dwmb-data-loader"
    echo "0/2 5-23 * * * \"/home/ubuntu/miniconda3/envs/comp30830py39_dudeWMB/bin/python ${module_to_schedule}\"" >> "${cron_dir}/dwmb-data-loader"
    echo "0 24 * * * \"/home/ubuntu/miniconda3/envs/comp30830py39_dudeWMB/bin/python ${module_to_schedule}\"" >> "${cron_dir}/dwmb-data-loader"
    exit 0
    ;;
stop)
    # shutdown: exit with a return code of 0
    echo "DWMB-Data-Loader.sh: About to erase the current Crontab..."
    read -r -p "                     Are you sure? [Y/N] " response
    if [[ "$response" =~ ^([yY][eE][sS]|[yY])$ ]]
    then
        #crontab -r
        if [ -e "${cron_dir}/dwmb-data-loader" ]; then
            rm "${cron_dir}/dwmb-data-loader"
            echo "DWMB-Data-Loader.sh: Crontab erased."
        else
            echo "DWMB-Data-Loader.sh: ERROR schedule not found! Aborting..."
            exit 1
        fi
...
*)
    ...
    errMsg="DWMB-Data-Loader.sh: ERROR Bad command. I don't understand \"$1\"\n";
